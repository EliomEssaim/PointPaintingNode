...
class Painter:
    def __init__(self, seg_net_index, calib_path=None):
        ...
        if seg_net_index == 0:
            ...
            print(f'Using Segmentation Network -- {SEG_NET_OPTIONS[seg_net_index]}')
            config_file = '../../painting/mmseg/configs/deeplabv3plus/deeplabv3plus_r18-d8_512x1024_80k_cityscapes.py'
            checkpoint_file = '../../painting/mmseg/checkpoints/deeplabv3plus_r18-d8_512x1024_80k_cityscapes_20201226_080942-cff257fe.pth'
            # config_file = '../../painting/mmseg/configs/deeplabv3plus/deeplabv3plus_r101-d8_512x1024_80k_cityscapes.py'
            # checkpoint_file = '../../painting/mmseg/checkpoints/deeplabv3plus_r101-d8_512x1024_80k_cityscapes_20200606_114143-068fcfe9.pth'
            self.model = init_segmentor(config_file, checkpoint_file, device=f'cuda:0') # TODO edit here if you want to use different device
    ...
    def get_calib_fromfilepath(self,file_path):
        calib_file = file_path
    ...
    def paint(self,points,image):
        ...
        scores_from_cam = self.get_score_by_image(image=image)
        scores_from_cam_r = scores_from_cam
        # scores_from_cam: H * W * 4/5, each pixel have 4/5 scores(0: background, 1: bicycle, 2: car, 3: person, 4: rider)

        # get calibration data
        calib_fromfile = self.get_calib_fromfilepath(self._calib_path)
    ...
    def run(self):
        img_list = os.listdir(os.path.join(self.root_split_path,'image_2'))
        num_image = len(img_list)
        img_list.sort()
        for idx in tqdm(range(num_image)):
            idx = int(img_list[idx].split('.')[0])
            sample_idx = "%06d" % idx
            # points: N * 4(x, y, z, r)
            points = self.get_lidar(sample_idx)
            
            # normalize reflect ratio @me
